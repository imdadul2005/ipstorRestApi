...
public class CommonAPI {
    ...
    public static List<String> getParsedStringList(String getAPI, String totalLocation, String ifEnabled, String find) throws IOException {
        JsonPath sanResouce = DataParser.rawToJSON(CommonAPI.commonGet(getAPI));
    ...
    public static void AssertCheck(JsonPath jp, Response rs, int expectedStatusCode, int expected_rc, int id) {
        try {
            logger.debug("Checking if the rc code is " + expected_rc);
            Assert.assertEquals(jp.get("rc"), expected_rc);
            logger.debug("Checking if the status code is " + expectedStatusCode);
            Assert.assertEquals(rs.statusCode(), expectedStatusCode);
        } catch (AssertionError e) {
            logger.error("!!! ERROR, EXPECTED RC CODE,STATUS CODE IS NOT RETURNED!!!");
            if(id>0)
                System.err.println("ERROR check log for detail for "+ id);
            else
                System.err.println("ERROR check log for detail");
        }
    }

    public static void AssertCheck(JsonPath jp, Response rs, int expectedStatusCode, int expected_rc) {
        AssertCheck(jp, rs, expectedStatusCode, expected_rc, -1);
    }
}